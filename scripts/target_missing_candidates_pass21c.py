...
OUTPUT_DIR = DATA / "missing_target_ocr_focus"
OUTPUT_LOG = DATA / "missing_target_pass21c.json"
...
def parse_hit_list(text):
    ...
    for sec in sections[1:]:
        ...
        approx_page = None
        for line in lines:
            line_ascii = "".join(ch for ch in line if ch.isascii())
            if "Approx." in line_ascii and "page" in line_ascii:
                m = re.search(r"\*\*(\d+)\*\*", line_ascii)
                if m:
                    approx_page = int(m.group(1))
                    break
        items.append(
            {
                "spec": spec,
...
def main():
    ...
    for item in items:
        ...
        approx_page = item.get("approxPage")
        number = item.get("number")
        kind = item.get("kind")
        if not spec or not pdf_name or not approx_page or not number:
        ...
        for page in range(max(1, approx_page - 20), approx_page + 21):
            ...
            out_txt = spec_dir / f"page-{page:04d}.txt"
    ...
    print("Target missing candidates focused scan complete.")
